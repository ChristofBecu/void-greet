...
public:
    ...
    [[nodiscard]] static Result<PersonName> create(std::string_view name) noexcept {
        ...
        // Check for invalid characters (more comprehensive validation)
        for (char c : name) {
            // Allow negative chars to pass through (for UTF-8 multibyte sequences)
            if (c >= 0) {
        ...
        // Additional validation: check for reasonable name patterns
...
public:
    ...
    [[nodiscard]] static Result<GreetingMessage> create(std::string_view message) noexcept {
        ...
        // Check for length constraints (warn if very long)
